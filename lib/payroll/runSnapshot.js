...
import { menitTelatUntukPayroll } from '../attendanceStatus';
...
// periode: 'YYYY-MM'.
// manual: { overtime, insentif, kompensasi } — input manual per periode, sama
// perannya dengan Lembur/Incentive/Compensation Fund di Payslip. Default 0
// (dipakai saat run baru dibuat, sebelum admin isi manual per employee).
...
export async function computeSnapshot(supabase, row, periode, manual = {}) {
  const gajiPokok = Number(row.gaji_pokok) || 0;
  const overtime = Number(manual.overtime) || 0;
  const insentif = Number(manual.insentif) || 0;
  const kompensasi = Number(manual.kompensasi) || 0;
  // Allowance yang DISIMPAN tetap base-only (Transport/Travel/Comm dari
  // employees_master) — supaya waktu di-generate jadi Payslip, tiap komponen
  // (Allowance/Overtime/Incentive/Compensation Fund) tetap kelihatan terpisah,
  // bukan tercampur jadi satu angka.
  const allowance = totalTunjangan(row);
  // Tapi rumus Bruto PPh21 & THP tetap menjumlahkan semuanya — sama seperti Payslip.
  const totalForBruto = allowance + overtime + insentif + kompensasi;
  ...
  if (row.linked_employee_id) {
    ...
    if (adaJadwal) {
      const totalMenitTelat = attRows.reduce((sum, r) => sum + menitTelatUntukPayroll(r, sched.jam_masuk), 0);
  ...
  const takeHomePay = gajiPokok + totalForBruto - penalty - bpjsEmployee - pph21;
