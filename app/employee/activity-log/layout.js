...
export default function ActivityLogLayout({ children }) {
  const router = useRouter();
  const supabase = createClient();
  // Superadmin-only (sama seperti Reports) — 'activity_log' sengaja tidak
  // ada di MODULE_OPTIONS jadi employee biasa tidak akan pernah bisa
  // di-grant akses ke key ini; hanya superadmin yang lolos.
  const { status, employee } = useModuleAccess('activity_log');
