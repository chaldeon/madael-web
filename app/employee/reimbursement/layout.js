...
export default function ReimbursementLayout({ children }) {
  const pathname = usePathname();
  const router = useRouter();
  const supabase = createClient();
  const { status, hasModule } = useModuleAccess(['reimbursement', 'reimbursement_admin']);
    ...
    return (
      <section className="min-h-screen flex items-center justify-center bg-[#F4F4F4] px-6">
        <div className="w-full max-w-[420px] border-t-4 border-madael-red bg-white p-8 text-center">
          <p className="text-sm text-black mb-6">Kamu tidak punya akses ke modul Reimbursement.</p>
          <Link
  ...
  const isAdmin = pathname.startsWith('/employee/reimbursement/admin');
        ...
        subnav={
          <>
            ...
            <div className="flex items-center gap-6">
              {hasModule('reimbursement') && (
                <Link href="/employee/reimbursement" className={tabClass(!isAdmin)}>
                  Ajukan Klaim
                </Link>
              )}
              {hasModule('reimbursement_admin') && (
                <Link href="/employee/reimbursement/admin" className={tabClass(isAdmin)}>
