...
export default function ProfileLayout({ children }) {
  ...
  const { status, hasModule } = useModuleAccess(['profile', 'profile_admin']);
    ...
    return (
      <section className="min-h-screen flex items-center justify-center bg-[#F4F4F4] px-6">
        <div className="w-full max-w-[420px] border-t-4 border-madael-red bg-white p-8 text-center">
          <p className="text-sm text-black mb-6">Kamu tidak punya akses ke modul Profil.</p>
          <Link
  ...
  const isAdmin = pathname.startsWith('/employee/profile/admin');
        ...
        subnav={
          <>
            ...
            <div className="flex items-center gap-6">
              {hasModule('profile') && (
                <Link href="/employee/profile" className={tabClass(!isAdmin)}>
                  Profil Saya
                </Link>
              )}
              {hasModule('profile_admin') && (
                <Link href="/employee/profile/admin" className={tabClass(isAdmin)}>
                  Kelola Perubahan Profil
                </Link>
              )}
            </div>
          </>
